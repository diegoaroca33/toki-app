...
export function EmergencyButton({user,personas}){
  ...
  function openSOS(){setShow(true);setTimeout(speakAll,500)}
  function closeSOS(){stopVoice();clearTimeout(speakTimer.current);setShow(false)}
  useEffect(()=>()=>{stopVoice();clearTimeout(speakTimer.current)},[]);
  if(!nombre)return null;
  return <>
    <button onClick={openSOS} style={{position:'fixed',bottom:20,right:20,width:72,height:72,borderRadius:'50%',border:'none',background:RED,color:'#fff',fontSize:36,cursor:'pointer',zIndex:90,display:'flex',alignItems:'center',justifyContent:'center',animation:'sosPulse 2s infinite',boxShadow:'0 4px 16px rgba(231,76,60,.4)'}}>🆘</button>
    {show&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,zIndex:500,background:RED,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:24,overflow:'auto'}}>
      <div style={{maxWidth:600,width:'100%',textAlign:'center'}}>
        ...
        <div style={{marginTop:20,display:'flex',flexDirection:'column',gap:12,alignItems:'center'}}>
          {telPadre&&<a href={'tel:'+telPadre.replace(/[\s.]/g,'')} style={{display:'inline-block',padding:'14px 28px',borderRadius:16,background:'rgba(255,255,255,.25)',border:'2px solid rgba(255,255,255,.6)',color:'#fff',fontSize:24,fontWeight:700,textDecoration:'none',fontFamily:"'Fredoka'",minWidth:220,textAlign:'center'}}>📞 Llamar{padre?' a '+padre.name:''}</a>}
          {telMadre&&telMadre!==telPadre&&<a href={'tel:'+telMadre.replace(/[\s.]/g,'')} style={{display:'inline-block',padding:'14px 28px',borderRadius:16,background:'rgba(255,255,255,.25)',border:'2px solid rgba(255,255,255,.6)',color:'#fff',fontSize:24,fontWeight:700,textDecoration:'none',fontFamily:"'Fredoka'",minWidth:220,textAlign:'center'}}>📞 Llamar{madre?' a '+madre.name:''}</a>}
          ...
          {direccion&&!showSafety&&<button onClick={()=>{setShowSafety(true);stopVoice();clearTimeout(speakTimer.current);
          ...
          }} style={{display:'inline-block',padding:'14px 28px',borderRadius:16,background:'rgba(255,255,255,.3)',border:'3px solid rgba(255,255,255,.8)',color:'#fff',fontSize:24,fontWeight:700,fontFamily:"'Fredoka'",minWidth:220,textAlign:'center',cursor:'pointer',boxShadow:'0 4px 16px rgba(0,0,0,.3)'}}>🏠 Ir a casa</button>}
        </div>
      </div>
      {showSafety&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(0,0,0,.85)',zIndex:510,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
        <div style={{maxWidth:420,width:'100%',background:'#1a237e',borderRadius:20,padding:'24px 20px',border:'3px solid #FFD700',textAlign:'left'}}>
          ...
          <button onClick={()=>{stopVoice();setGettingLoc(true);
          ...
          <button onClick={()=>{setShowSafety(false);stopVoice()}} style={{width:'100%',padding:'10px',borderRadius:10,background:'transparent',border:'2px solid rgba(255,255,255,.3)',color:'rgba(255,255,255,.7)',fontSize:16,fontWeight:600,fontFamily:"'Fredoka'",cursor:'pointer',marginTop:8,textAlign:'center'}}>← Volver</button>
        </div>
      </div>}
      <div style={{position:'absolute',bottom:30,display:'flex',flexDirection:'column',alignItems:'center',gap:8}}>
        <button onClick={()=>{setShowSafety(false);closeSOS()}} style={{background:'rgba(0,0,0,.3)',border:'2px solid rgba(255,255,255,.4)',borderRadius:12,padding:'12px 28px',color:'#fff',fontSize:20,fontWeight:600,fontFamily:"'Fredoka'",cursor:'pointer'}}>← Volver</button>
      </div>
    </div>}
  </>}
