            ...
            return dynGroups.map(g=>{
              ...
              return <div key={g.id} style={{marginBottom:10,border:`1px solid ${g.color+'33'}`,borderRadius:10,padding:12,background:g.color+'06'}}>
                ...
                :g.modules.map((m,mi)=>{
                  ...
                  </div>:(()=>{const singleLv=opts[0]||{n:Array.isArray(m.defLv)?m.defLv[0]:m.defLv,l:m.l};const isOn=curLvs.includes(singleLv.n);return <button onClick={()=>{setModuleLv(m.lvKey,isOn?[]:[singleLv.n]);const na={...activeMods,[m.lvKey]:!isOn};setActiveMods(na);saveData('active_mods',na)}} style={{padding:'6px 14px',borderRadius:8,border:`2px solid ${isOn?GREEN:BORDER}`,background:isOn?GREEN+'22':BG3+'44',color:isOn?GREEN:DIM,fontFamily:"'Fredoka'",fontWeight:600,fontSize:13,cursor:'pointer',minHeight:44}}>{isOn?'✓ Activo':'Desactivado'}</button>})()}
                </div>})}
              </div>})})()}</div>}</div>
