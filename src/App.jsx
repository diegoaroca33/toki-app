...
const AVS=['🧑‍🚀','🦸','🧙','🐉','🤖','🦊','🎮','⚡','🌟','🐸'];const CLS=[GREEN,BLUE,GOLD,PURPLE,RED,'#E67E22',GREEN];const tdy=()=>new Date().toLocaleDateString('es-ES');const rnd=a=>a[Math.floor(Math.random()*a.length)];
...
function SpeakPanel({text,exId,onOk,onSkip,sex,name,uid,vids}){
  ...
  const dur=useMemo(()=>Math.max(3,Math.ceil(text.split(/\s+/).length*0.6)+2),[text]);
  async function doPlay(){if(!alive.current)return;stopVoice();sMsg('');setMic(false);
    ...
    if(!alive.current)return;await new Promise(r=>setTimeout(r,400));if(!alive.current)return;setMic(true);setTimeout(()=>{if(alive.current)sr.go()},200)}
...
function VoiceRec({user,onBack,onSave}){const[mode,setMode]=useState('menu');const[recLv,setRecLv]=useState(1);const[selV,setSelV]=useState(null);const[vn,setVn]=useState('');const[va,setVa]=useState('🎙️');const[vs,setVs]=useState('m');const[ri,setRi]=useState(0);const[rec,setRec]=useState(false);const[mr,setMr]=useState(null);const[saved,setSaved]=useState(0);const[pp,setPp]=useState(-1);const ch=useRef([]);const vid=useRef(null);const voices=user.voices||[];
  function init(ex){if(ex){setSelV(ex);vid.current=ex.id;setVn(ex.name);setVa(ex.avatar);setVs(ex.sex);setSaved(ex.saved||0)}else{setSelV(null);vid.current=Date.now()+'';setSaved(0)}}
  const cheerItems=useMemo(()=>[...(user.sex==='f'?PERFECT_F:PERFECT_M),...GOOD_MSG,...RETRY_MSG,...FAIL_MSG,...BUILD_OK],[user.sex]);
  const phraseItems=useMemo(()=>EX.filter(e=>e.lv===recLv).map(e=>e.ph||e.fu||e.su).filter(Boolean),[recLv]);
  const items=mode==='cheers'?cheerItems:phraseItems;const cur=items[ri];
  async function startR(){try{const s=await navigator.mediaDevices.getUserMedia({audio:{sampleRate:16000,channelCount:1,echoCancellation:true}});const m=new MediaRecorder(s,{mimeType:MediaRecorder.isTypeSupported('audio/webm;codecs=opus')?'audio/webm;codecs=opus':'audio/webm',audioBitsPerSecond:32000});ch.current=[];m.ondataavailable=e=>{if(e.data.size>0)ch.current.push(e.data)};m.onstop=()=>{const b=new Blob(ch.current,{type:'audio/webm'});const r=new FileReader();r.onload=()=>{const k=mode==='cheers'?'cheer_'+ri:'phrase_'+recLv+'_'+ri;const sk='voice_'+user.id+'_'+vid.current;const d=loadData(sk,{});d[k]=r.result;d.name=vn;d.avatar=va;d.sex=vs;saveData(sk,d);setSaved(sv=>sv+1)};r.readAsDataURL(b);s.getTracks().forEach(t=>t.stop())};m.start();setMr(m);setRec(true)}catch(e){alert('No se puede acceder al micrófono')}}
  function stopR(){if(mr){mr.stop();setMr(null);setRec(false)}}
  function preview(i){const k=mode==='cheers'?'cheer_'+i:'phrase_'+recLv+'_'+i;try{const d=loadData('voice_'+user.id+'_'+vid.current,{});if(d[k]){setPp(i);const a=new Audio(d[k]);a.onended=()=>setPp(-1);a.play().catch(()=>setPp(-1))}}catch(e){}}
...
export default function App(){
  ...
  return <div onClick={tU} onTouchStart={tU}><style>{CSS}</style><Confetti show={conf}/>
    ...
    {ov==='parentGate'&&user&&<div className="ov"><div className="ovp"><div style={{fontSize:48,marginBottom:12}}>👨‍👩‍👦</div><p style={{fontSize:20,fontWeight:700,margin:'0 0 8px'}}>Panel de Padres</p><p style={{fontSize:14,color:DIM,margin:'0 0 14px'}}>Introduce el PIN</p><input className="inp" value={parentPin} onChange={e=>setParentPin(e.target.value.replace(/\D/g,'').slice(0,4))} type="tel" placeholder="· · · ·" style={{textAlign:'center',fontSize:30,letterSpacing:16,borderColor:pe?RED:BORDER}}/><div style={{display:'flex',gap:10,marginTop:16}}><button className="btn btn-ghost" style={{flex:1}} onClick={()=>{setOv(null);setParentPin('')}}>Cancelar</button><button className="btn btn-g" style={{flex:1}} disabled={parentPin.length<4} onClick={()=>{if(parentPin===user.pin){setParentPin('');setOv('parent')}else{setPe(true);setParentPin('');setTimeout(()=>setPe(false),1500)}}}>Entrar</button></div></div></div>}
    {ov==='parent'&&user&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:BG,overflowY:'auto',zIndex:100,padding:16}}><div style={{maxWidth:600,margin:'0 auto'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}><p style={{fontSize:20,color:GOLD,fontWeight:700,margin:0}}>👨‍👩‍👦 Panel</p><button className="btn btn-ghost btn-half" style={{width:'auto',padding:'8px 16px'}} onClick={()=>setOv(null)}>✕</button></div>
      ...
      {ptab==='config'&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
        ...
        <button className="btn btn-gold" onClick={()=>{const up={...user,pin:pp,sessionMin:sm};setUser(up);saveP(up);setOv(null)}}>💾 Guardar</button>
    ...
    {scr==='login'&&<div className="af" style={{textAlign:'center',padding:'24px 0'}}><div style={{fontSize:80,marginBottom:8,animation:'glow 3s infinite'}}>🗣️</div><h1 style={{fontSize:44,color:GOLD,margin:'0 0 4px',letterSpacing:-1}}>Toki</h1><p style={{color:DIM,fontSize:16,margin:'0 0 32px',fontStyle:'italic'}}>Aprende a decirlo</p><p style={{color:DIM+'66',fontSize:10,position:'fixed',bottom:10,left:0,right:0,textAlign:'center'}}>© 2026 Toki — Diego Aroca</p>
      {profs.length>0&&!creating&&<div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:24}}>{profs.map(p=><button key={p.id} className="profcard" onClick={()=>{setUser(p);setPp(p.pin||'');setSm(p.sessionMin||25);setVoiceProfile(p.age,p.sex);setScr('goals')}}><div style={{fontSize:40}}>{p.av}</div><div style={{flex:1}}><div style={{fontSize:22,fontWeight:700}}>{p.name}</div><div style={{fontSize:14,color:DIM}}>{p.age} años · Nivel {p.maxLv||p.level||1}</div></div></button>)}</div>}
      ...
      {creating&&<div className="card af" style={{padding:24,textAlign:'left'}}><p style={{fontSize:22,color:GOLD,textAlign:'center',margin:'0 0 18px',fontWeight:700}}>Nuevo Jugador</p>
        ...
        <div style={{display:'flex',gap:10}}><button className="btn btn-ghost" style={{flex:1}} onClick={()=>setCreating(false)}>Cancelar</button><button className="btn btn-g" style={{flex:2}} disabled={!fn.trim()||!fa||pp.length<4} onClick={()=>{const bd=new Date(fa),now=new Date(),age=Math.floor((now-bd)/31557600000);const p={id:Date.now()+'',name:cap(fn.trim()),birthdate:fa,age:Math.max(1,age),sex:fsex,av:fav,hist:[],srs:{},level:flv,maxLv:flv,pin:pp,sessionMin:25,voices:[],padre:fPadre.trim(),madre:fMadre.trim(),hermanos:fHerm.trim(),telefono:fTel.trim(),direccion:fDir.trim()};setProfs(prev=>[...prev,p]);setUser(p);setCreating(false);setFn('');setFa('');setPp('');setFPadre('');setFMadre('');setFHerm('');setFTel('');setFDir('');setVoiceProfile(Math.max(1,age),fsex);setScr('goals')}}>Crear ✓</button></div></div>}
    </div>}

    {scr==='goals'&&user&&<div className="af"><div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}><button style={{background:'none',border:'none',color:DIM,fontSize:16}} onClick={()=>{setUser(null);setScr('login')}}>← Cambiar</button><div style={{display:'flex',gap:12}}><button style={{background:'none',border:'none',color:GOLD,fontSize:16}} onClick={()=>setShowRec(true)}>🎙️</button><button style={{background:'none',border:'none',color:DIM,fontSize:16}} onClick={()=>{setParentPinOk(false);setParentPin('');setPp(user.pin||'');setSm(user.sessionMin||25);setPtab('config');setDelConf(false);setOv('parentGate')}}>⚙️</button></div></div>
      <div style={{textAlign:'center',padding:'20px 0'}}><div style={{fontSize:56,marginBottom:8}}>{user.av}</div><h2 style={{fontSize:28,margin:'0 0 4px'}}>¡Hola {user.name}!</h2><p style={{fontSize:15,color:DIM,margin:'0 0 20px'}}>Nivel {user.maxLv||user.level||1} de 5</p>
