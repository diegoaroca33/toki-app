...
// ===== COLORS (azul profundo + dorado cálido + verdes de éxito) =====
...
.btn-r{background:${RED};border-color:#c0392b;box-shadow:4px 4px 0 #922b21}
...
.btn-inline{display:inline-block;width:auto;padding:14px 20px;font-size:18px}
.tag{display:inline-block;padding:6px 16px;border-radius:24px;font-size:15px;font-weight:700}
...
.ws{min-width:56px;height:52px;display:flex;align-items:center;justify-content:center;padding:0 14px;border-radius:12px;font-size:20px;font-weight:700;transition:all .2s}
...
// Mobile requires user gesture to unlock audio
...
function speak(text,exId){
  return new Promise(res=>{
    if(exId){const a=new Audio(`/audio/${voiceProfile.sex}/${exId}.mp3`);a.onended=res;a.onerror=()=>tts(text).then(res);a.play().catch(()=>tts(text).then(res));return}
    tts(text).then(res);
  });
}
function tts(text){
  return new Promise(res=>{
    ...
    window.speechSynthesis.cancel();if(!cachedVoice)pickVoice();
    const p=getVP(),u=new SpeechSynthesisUtterance(text);u.lang='es-ES';u.rate=p.rate;u.pitch=p.pitch;
    if(cachedVoice)u.voice=cachedVoice;u.onend=res;u.onerror=res;window.speechSynthesis.speak(u);setTimeout(res,6000);
  });
}
...
const CHEERS=['¡Muy bien! 🌟','¡Genial! ⭐','¡Qué crack! 💪','¡Perfecto! 🎉','¡Así se habla! 🔥','¡Bravo! ✨','¡Espectacular! 🏆'];
const ALMOSTS=['¡Casi! Escúchalo otra vez 👂','¡Tú puedes! Una más 🎯'];
const GOODS=['¡Buen intento! Seguimos 👏','¡Bien hecho! A por otra 🚀','¡Vas genial! ⭐'];
const rnd=a=>a[Math.floor(Math.random()*a.length)];

// ===== SMALL COMPONENTS =====
function Tag({c,ch}){const cl=c||GREEN;return <span className="tag" style={{background:cl+'22',color:cl,border:`2px solid ${cl}44`}}>{ch}</span>}
...
function Tower({placed,total}){const cells=21,filled=Math.min(Math.floor((placed/Math.max(total,1))*cells),cells);return <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:3,maxWidth:220,margin:'0 auto'}}>{Array.from({length:cells},(_,i)=>{const row=Math.floor(i/7),inv=(2-row)*7+(i%7),on=inv<filled;return <div key={i} style={{aspectRatio:'1',borderRadius:4,transition:'all .3s cubic-bezier(.34,1.56,.64,1)',background:on?CLS[inv%7]:BG3+'44',border:on?'2px solid rgba(0,0,0,.2)':'2px solid '+BG3,boxShadow:on?'inset -1px -1px 0 rgba(0,0,0,.15),inset 1px 1px 0 rgba(255,255,255,.15)':'none',transform:on?'scale(1)':'scale(.75)',opacity:on?1:.3}}/>})}</div>}

// ===== SPEAK PANEL — shared by all exercise types =====
// Shows: phrase, "🚀 ¡Vamos!" button (plays+listens), "🔊 Oír otra vez", feedback
function SpeakPanel({text,exId,onOk,onSkip}){
  const[sf,sSf]=useState(null);const[att,sAtt]=useState(0);const[msg,sMsg]=useState('');
  const sr=useSR();
  useEffect(()=>{sSf(null);sAtt(0);sMsg('')},[text]);
  useEffect(()=>{if(!sr.res)return;
    const b=Math.max(...sr.res.split('|').map(a=>score(a,text)));
    if(b>=2){sMsg(rnd(CHEERS));sSf('ok');setTimeout(onOk,1200)}
    else{const na=att+1;sAtt(na);if(na>=2){sMsg(rnd(GOODS));sSf('pass');setTimeout(onOk,1200)}
    else{sMsg(rnd(ALMOSTS));sSf('re');setTimeout(()=>{sSf(null);sMsg('')},1600)}}
  },[sr.res]);
  function goSpeak(){unlockAudio();speak(text,exId).then(()=>{sMsg('¡Ahora tú! 🎤');setTimeout(()=>sr.go(),300)})}
  function hearAgain(){unlockAudio();speak(text,exId)}
  return <div>
    <div className="card" style={{padding:22,marginBottom:16,background:GREEN+'0C',borderColor:GREEN+'33'}}>
      <p style={{fontSize:24,fontWeight:700,margin:0,lineHeight:1.4,textAlign:'center'}}>"{text}"</p>
    </div>
    {msg&&<div className={sf==='ok'||sf==='pass'?'ab':'as'} style={{background:sf==='ok'?GREEN+'20':sf==='pass'?BLUE+'20':RED+'20',borderRadius:14,padding:16,marginBottom:14,textAlign:'center'}}>
      <p style={{fontSize:20,fontWeight:700,margin:0,color:sf==='ok'?GREEN:sf==='pass'?BLUE:GOLD}}>{msg}</p>
    </div>}
    {!sf&&<div style={{display:'flex',flexDirection:'column',gap:12}}>
      {sr.on?
        <div style={{textAlign:'center',padding:16}}><span className="ap" style={{display:'inline-block',fontSize:48}}>🎤</span><p style={{fontSize:18,color:RED,fontWeight:700,margin:'8px 0 0'}}>Te escucho...</p></div>
      :<>
        <button className="btn btn-g" onClick={goSpeak} style={{fontSize:24,padding:20}}>🚀 ¡Vamos!</button>
        <button className="btn btn-b btn-inline" onClick={hearAgain} style={{margin:'0 auto',fontSize:16,padding:'12px 24px'}}>🔊 Oír otra vez</button>
      </>}
    </div>}
    <div style={{marginTop:14}}><button className="btn btn-ghost" onClick={onSkip}>⏭️ Saltar</button></div>
  </div>
}
...
function ExFrases({ex,onOk,onSkip}){
  ...
  function place(item){unlockAudio();const s=pl.findIndex(p=>p===null);if(s===-1)return;const np=[...pl];np[s]=item;sPl(np);sAv(a=>a.map(x=>x.i===item.i?{...x,u:true}:x));
    if(np.every(p=>p!==null)){const built=np.map(p=>p.w.toLowerCase()).join(' ');const target=words.map(w=>w.toLowerCase()).join(' ');
      if(built===target){sBf('ok');speak(ex.fu,ex.id).then(()=>sPh('speak'))}
  ...
  return <div style={{textAlign:'center',padding:18}}>
    <div style={{fontSize:64,marginBottom:12,animation:'glow 3s infinite'}}>{ex.em}</div>
    {ph==='build'&&<div className="af">
      <div className="card" style={{marginBottom:16,background:BLUE+'0C',borderColor:BLUE+'33'}}><p style={{fontSize:20,fontWeight:600,margin:0,lineHeight:1.4,color:BLUE}}>{ex.q}</p></div>
      ...
      {bf==='ok'&&<div className="ab" style={{background:GREEN+'20',borderRadius:14,padding:16,marginBottom:12}}><span style={{fontSize:36}}>⭐</span><p style={{fontSize:18,color:GREEN,fontWeight:700,margin:'6px 0 0'}}>¡Bien!</p></div>}
      {bf==='no'&&<div className="as" style={{background:RED+'20',borderRadius:14,padding:14,marginBottom:12}}><p style={{fontSize:16,color:GOLD,fontWeight:700,margin:0}}>¡Casi! Inténtalo 💪</p></div>}
      {!bf&&<div style={{display:'flex',flexWrap:'wrap',gap:10,justifyContent:'center',marginBottom:14}}>{av.filter(x=>!x.u).map(x=><button key={x.i} className="btn btn-b btn-inline" onClick={()=>place(x)} style={{fontSize:19}}>{x.w}</button>)}</div>}
      <div style={{display:'flex',gap:10,justifyContent:'center'}}>
        {!bf&&pl.some(p=>p)&&<button className="btn btn-o btn-inline" onClick={undo} style={{fontSize:16}}>↩️ Borrar</button>}
        <button className="btn btn-p btn-inline" onClick={()=>{unlockAudio();speak(ex.fu,ex.id)}} style={{fontSize:16}}>🔊 Pista</button>
      </div>
      <div style={{marginTop:14}}><button className="btn btn-ghost" onClick={onSkip}>⏭️ Saltar</button></div>
    </div>}
    {ph==='speak'&&<SpeakPanel text={ex.fu} exId={ex.id} onOk={onOk} onSkip={onSkip}/>}
  </div>
}
...
function ExSit({ex,onOk,onSkip}){
  ...
  return <div style={{textAlign:'center',padding:18}}>
    <div style={{fontSize:64,marginBottom:12}}>{ex.em}</div>
    <div className="card" style={{marginBottom:16,background:BLUE+'0C',borderColor:BLUE+'33'}}><Tag c={BLUE} ch="SITUACIÓN"/><p style={{fontSize:18,fontWeight:600,margin:'10px 0 0',lineHeight:1.4}}>{ex.si}</p></div>
    {ph==='choose'&&<div className="af">
      <p style={{fontSize:18,color:GOLD,fontWeight:700,margin:'0 0 12px'}}>¿Qué dirías?</p>
      {cf==='no'&&<div className="as" style={{background:RED+'20',borderRadius:12,padding:10,marginBottom:10}}><p style={{fontSize:16,color:GOLD,margin:0}}>Piensa... 🤔</p></div>}
      <div style={{display:'flex',flexDirection:'column',gap:12}}>
        {shuf.map((o,i)=><button key={i} className="btn btn-b" onClick={()=>{unlockAudio();if(o===ex.op[0]){speak(ex.su,ex.id);setTimeout(()=>sPh('speak'),800)}else{sCf('no');setTimeout(()=>sCf(null),1000)}}} style={{textAlign:'left',fontSize:18}}>{o}</button>)}
      </div>
      <div style={{marginTop:14}}><button className="btn btn-ghost" onClick={onSkip}>⏭️ Saltar</button></div>
    </div>}
    {ph==='speak'&&<SpeakPanel text={ex.su} exId={ex.id} onOk={onOk} onSkip={onSkip}/>}
  </div>
}
...
function ExFlu({ex,onOk,onSkip}){
  return <div style={{textAlign:'center',padding:18}}>
    <div style={{fontSize:64,marginBottom:12,animation:'glow 3s infinite'}}>{ex.em}</div>
    <SpeakPanel text={ex.ph} exId={ex.id} onOk={onOk} onSkip={onSkip}/>
  </div>
}
...
export default function App(){
  ...
  function onCorrect(){setConf(true);setTimeout(()=>setConf(false),2400);const e=queue[idx];const up=srsUp(e.id,true,user);if(!up.seen)up.seen=[];if(!up.seen.includes(e.id))up.seen.push(e.id);setUser(up);saveP(up);const ns={ok:st.ok+1,sk:st.sk};setSt(ns);setTimeout(()=>goNext(ns),200)}
  ...
  return <div onClick={unlockAudio}>
    ...
    {ov==='parent'&&user&&<div style={{position:'fixed',top:0,left:0,right:0,bottom:0,background:BG,overflowY:'auto',zIndex:100,padding:16}}>
      <div style={{maxWidth:500,margin:'0 auto'}}><div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:18}}><p style={{fontSize:20,color:GOLD,fontWeight:700,margin:0}}>👨‍👩‍👦 Panel de Padres</p><button className="btn btn-ghost btn-inline" style={{padding:'8px 14px',fontSize:14}} onClick={()=>setOv(null)}>✕ Cerrar</button></div>
      <div className="tabs" style={{marginBottom:18}}>{['config','stats','srs'].map(t=><button key={t} className={'tab'+(ptab===t?' on':'')} onClick={()=>setPtab(t)}>{t==='config'?'⚙️ Ajustes':t==='stats'?'📊 Datos':'🧠 SRS'}</button>)}</div>
      {ptab==='config'&&<div style={{display:'flex',flexDirection:'column',gap:16}}>
        <div className="card"><p style={{fontSize:16,fontWeight:700,margin:'0 0 10px'}}>🔒 PIN de salida</p><input className="inp" value={pp} onChange={e=>setPp(e.target.value.replace(/\D/g,'').slice(0,4))} type="tel" placeholder="1234" style={{textAlign:'center',fontSize:24,letterSpacing:12}}/></div>
        <div className="card"><p style={{fontSize:16,fontWeight:700,margin:'0 0 10px'}}>🎯 Ejercicios/sesión: <span style={{color:GREEN}}>{pg}</span></p><input type="range" min="4" max="20" value={pg} onChange={e=>setPg(+e.target.value)} style={{width:'100%',accentColor:GREEN}}/></div>
      ...
      {ptab==='stats'&&(()=>{const ss=user.hist||[],tc=ss.reduce((s,h)=>s+h.ok,0),ta=ss.reduce((s,h)=>s+h.ok+h.sk,0),pct=ta>0?Math.round(tc/ta*100):0;return <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        {[{l:'Sesiones',v:ss.length,c:GREEN},{l:'Aciertos',v:tc,c:BLUE},{l:'% Acierto',v:pct+'%',c:GOLD},{l:'Total',v:ta,c:PURPLE}].map((s,i)=><div key={i} className="sbox"><div style={{fontSize:28,color:s.c,fontWeight:700}}>{s.v}</div><div style={{fontSize:13,color:DIM,marginTop:4}}>{s.l}</div></div>)}
      </div>})()}
    ...
    {scr==='login'&&<div className="af" style={{textAlign:'center',padding:'24px 0'}}>
      <div style={{fontSize:72,marginBottom:8,animation:'glow 3s infinite'}}>🗣️</div>
      <h1 style={{fontSize:42,color:GOLD,margin:'0 0 4px',letterSpacing:-1}}>Toki</h1>
      <p style={{color:DIM,fontSize:16,margin:'0 0 32px',fontStyle:'italic'}}>Aprende a decirlo</p>
      <p style={{color:BG3,fontSize:10,position:'fixed',bottom:10,left:0,right:0,textAlign:'center',margin:0}}>© 2026 Toki · Aprende a decirlo — Diego Aroca</p>
      {profs.length>0&&!creating&&<div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:24}}>{profs.map(p=><button key={p.id} className="profcard" onClick={()=>{setUser(p);setPp(p.pin||'');setPg(p.goal||8);setVoiceProfile(p.age,p.sex);setScr('goals')}}>
        <div style={{fontSize:40}}>{p.av}</div><div style={{flex:1}}><div style={{fontSize:20,fontWeight:700}}>{p.name}</div><div style={{fontSize:14,color:DIM}}>{p.age} años · Nivel {p.maxLv||p.level||1}</div></div>
      </button>)}</div>}
      ...
      <div className="card af" style={{padding:24,textAlign:'left'}}>
        <p style={{fontSize:20,color:GOLD,textAlign:'center',margin:'0 0 18px',fontWeight:700}}>Nuevo Jugador</p>
        ...
        <label style={{fontSize:14,color:DIM}}>Edad</label>
        ...
        <label style={{fontSize:14,color:DIM}}>Sexo (para la voz)</label>
        ...
        <label style={{fontSize:14,color:DIM}}>Nivel</label>
        <div style={{display:'flex',gap:8,margin:'8px 0 6px'}}>{[1,2,3,4,5].map(n=><button key={n} onClick={()=>setFlv(n)} style={{flex:1,padding:'14px 0',borderRadius:10,border:`3px solid ${flv===n?GOLD:BORDER}`,background:flv===n?GOLD+'22':BG3,color:flv===n?GOLD:DIM,fontFamily:"'Fredoka'",fontWeight:700,fontSize:18,cursor:'pointer'}}>{n}</button>)}</div>
        <p style={{fontSize:12,color:DIM,marginBottom:14,textAlign:'center'}}>{flv===1?'Palabras cortas':flv===2?'Frases diarias':flv===3?'Relaciones':flv===4?'Autonomía':'Expresión rica'}</p>
        <label style={{fontSize:14,color:DIM}}>🔒 PIN (opcional)</label>
        <input className="inp" value={pp} onChange={e=>setPp(e.target.value.replace(/\D/g,'').slice(0,4))} type="tel" placeholder="Sin PIN = puede salir" style={{marginBottom:14,marginTop:6,textAlign:'center',fontSize:22,letterSpacing:12}}/>
        <label style={{fontSize:14,color:DIM}}>Avatar</label>
    ...
    {scr==='goals'&&user&&<div className="af">
      ...
      <div style={{textAlign:'center',padding:'20px 0'}}>
        ...
        <h2 style={{fontSize:26,margin:'0 0 4px'}}>¡Hola {user.name}!</h2>
        <p style={{fontSize:14,color:DIM,margin:'0 0 6px'}}>Nivel {user.maxLv||user.level||1} de 5</p>
        {(()=>{const rem=getRem(),goal=user.goal||8,done=goal-rem;
          if(rem>0)return <div><div style={{background:GREEN+'0C',border:`2px solid ${GREEN}22`,borderRadius:18,padding:24,margin:'20px 0'}}>
            ...
            <p style={{fontSize:18,fontWeight:700,margin:'0 0 4px'}}>Te faltan <span style={{color:GREEN}}>{rem} ejercicios</span></p>
            <p style={{fontSize:15,color:DIM,margin:0}}>¡Termínalos y la tablet es tuya! 🎮</p></div>
            <button className="btn btn-g" onClick={startGame} style={{fontSize:22}}>🚀 ¡A por ello!</button></div>;
          return <div><div style={{background:GOLD+'0C',border:`2px solid ${GOLD}22`,borderRadius:18,padding:24,margin:'20px 0'}}>
            <div style={{fontSize:64,marginBottom:8}}>🏆</div><p style={{fontSize:22,color:GOLD,fontWeight:700,margin:'0 0 8px'}}>¡SESIÓN COMPLETADA!</p><p style={{fontSize:15,color:DIM,margin:0}}>¡Crack!</p></div>
    ...
    {scr==='game'&&cur&&<div className="af">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
        ...
        <Tag c={GOLD} ch={`${idx+1}/${queue.length}`}/>
      </div>
      <div className="pbar" style={{marginBottom:12}}><div className="pfill" style={{width:(idx/queue.length*100)+'%'}}/></div>
      <Tower placed={st.ok} total={queue.length}/>
      <div style={{textAlign:'center',margin:'10px 0 6px'}}>
        <Tag c={cur.ty==='frases'?GREEN:cur.ty==='sit'?BLUE:PURPLE} ch={cur.ty==='frases'?'🧱 Haz la Frase':cur.ty==='sit'?'💬 ¿Qué Dices?':'🎤 Dilo Seguido'}/>{' '}
        {area&&<Tag c={DIM} ch={area.ic+' '+area.n}/>}
      </div>
