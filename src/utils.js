...
export function adjScore(raw){const ex=getExigencia();if(ex>=100)return raw;return Math.min(4,Math.round(raw*(100/ex)))}
export function cap(s){return s.charAt(0).toUpperCase()+s.slice(1).toLowerCase()}
export function saveData(key,val){try{localStorage.setItem('toki_'+key,JSON.stringify(val,(k,v)=>{if(v instanceof HTMLElement||v instanceof Node)return undefined;if(typeof v==='object'&&v!==null&&v.$$typeof)return undefined;return v}))}catch(e){console.warn('[Toki] saveData error:',key,e)}}
